...
Now, make sure that in `Properties -> Configuration Properties -> C/C++ -> General -> Additional Include Directories` of your C++ project in Visual Studio, you have include the folder you pasted the `glm` folder in. This is because the glm library's source code uses paths relative to the directory containing this `glm` folder.
